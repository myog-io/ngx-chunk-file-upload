...
export class FileUploaderService {
			...
			this.onBeforeGetDefaultHeaders().then(
				(result) => {
					...
					if (this.options.tokenPattern && this.options.token) {
						h[this.options.authorizationHeaderName] = this.options.tokenPattern.replace(
							'#token#',
							this.options.token,
						);
					}
	...
	public buildPackageToSend(item: FileItem, options: FileUploaderOptions) {
		const sendable: FormData = new FormData();
	...
	public deleteEntry(
		...
		if (item.getId() && this.links['deleteEntry']) {
			const link = this.links['deleteEntry'].replace(/#id#/g, item.getId());
